...
class BasicAI:
    ...
    def __init__(self):
        ...
        self.totalAllies = {'B': 6, '1': 1, '2': 1, '3': 2, '4': 3,
                            '5': 4, '6': 4, '7': 4, '8': 5, '9': 8, 's': 1, 'F': 1}
        self.totalEnemies = {'B': 6, '1': 1, '2': 1, '3': 2, '4': 3,
                             '5': 4, '6': 4, '7': 4, '8': 5, '9': 8, 's': 1, 'F': 1}
        self.hiddenEnemies = {'B': 6, '1': 1, '2': 1, '3': 2, '4': 3,
                              '5': 4, '6': 4, '7': 4, '8': 5, '9': 8, 's': 1, 'F': 1}
        self.hiddenAllies = {'B': 6, '1': 1, '2': 1, '3': 2, '4': 3,
                             '5': 4, '6': 4, '7': 4, '8': 5, '9': 8, 's': 1, 'F': 1}
        self.lastMoved = None
    ...
    def Setup(self):
        ...
        if len(setup) < 4:
            sys.stderr.write("BasicAI setup fails, expected at least 4 tokens, got " +
                             str(len(setup)) + " "+str(setup) + "\n")
        self.colour = setup[0]
                    ...
                    if p[0] >= 0 and p[0] < self.width and p[1] >= 0 and p[1] < self.height:
                        ...
                        if target == None or (target.colour != piece.colour and target.colour != "NONE" and target.colour != "BOTH"):
                            print(str(piece.x) + " " + str(piece.y) +
                                  " "+directions[dirIndex])
                            f = open("moves","w")
                            f.write(str(piece.x) + " " + str(piece.y) + " "+directions[dirIndex])
                            f.close()
                            return True
    ...
    def ReadBoard(self):
        ...
        for y in range(0, self.height):
            ...
            self.board2.append(row)
            if len(row) < self.width:
                sys.stderr.write(
                    "Row has length " + str(len(row)) + " vs " + str(self.width) + "\n")
                return False
            for x in range(0, self.width):
                if self.turn == 0:
                    if row[x] == '.':
                        ...
                        self.board[x][y] = Piece(
                            oppositeColour(self.colour), '?', x, y)
