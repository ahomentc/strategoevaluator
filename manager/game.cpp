...
Game::Game(const char * redPath, const char * bluePath, const char * red_setup, const char * blue_setup, const bool enableGraphics, double newStallTime, const bool allowIllegal, FILE * newLog, const  Piece::Colour & newReveal, int newMaxTurns, bool newPrintBoard, double newTimeoutTime, const char * newImageOutput) : red(NULL), blue(NULL), turn(Piece::RED), theBoard(10,10), graphicsEnabled(enableGraphics), stallTime(newStallTime), allowIllegalMoves(allowIllegal), log(newLog), reveal(newReveal), turnCount(0), input(NULL), maxTurns(newMaxTurns), printBoard(newPrintBoard), timeoutTime(newTimeoutTime), imageOutput(newImageOutput)
{
	gameCreated = false;
	if (gameCreated)
	{
	...
	MakeControllers(redPath, bluePath, red_setup, blue_setup);
 ...
 */
void Game::MakeControllers(const char * redPath, const char * bluePath, const char * red_setup, const char * blue_setup)
{
	...
	if (redPath[0] == '@')
	{
		...
		red = new AI_Controller(Piece::RED, redPath, timeoutTime, red_setup);
	...
	if (bluePath[0] == '@')
	{
		...
		blue = new AI_Controller(Piece::BLUE, bluePath, timeoutTime, blue_setup);
