 ...
 */
Program::Program(const char * executablePath, const char * new_initial_setup) : input(NULL), output(NULL), pid(0), paused(false)
{
	...
	*/
	...
	if (pid == 0)
	{
		...
		{
			execl(executablePath, executablePath, new_initial_setup, (char*)(NULL)); ///Replace process with desired executable
			//execv(executablePath,(char*)(NULL)); ///Replace process with desired executable
		}
		perror("execv error:\n");
